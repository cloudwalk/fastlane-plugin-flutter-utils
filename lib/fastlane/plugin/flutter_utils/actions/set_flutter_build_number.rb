...
module Fastlane
  module Actions
    class SetFlutterBuildNumberAction < Action
      def self.run(params)
        helper = Helper::FlutterUtilsHelper.new
        helper.set_flutter_build_number(build: params[:build], pubspec_path: params[:pubspec_path])
      end
